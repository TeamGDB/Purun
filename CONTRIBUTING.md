# Contributing to Purun

Thank you for wanting to help. Purun is a work in progress; [`docs/MISSING.md`](docs/MISSING.md) says what is missing, most blocking first, and the [issues](https://github.com/TeamGDB/Purun/issues) are the work. These rules are the short form of [AGENTS.md](AGENTS.md) and of [PortableKit's contributing guide](https://github.com/TeamGDB/PortableKit/blob/main/CONTRIBUTING.md), which apply here too.

## Where a change belongs

Almost nothing lives in this repository: it is the game's profile, `host/purun_profile.cpp`. **A change that is true of the PSP, of a file format, or of any game that makes the same call belongs in [PortableKit](https://github.com/TeamGDB/PortableKit)**, as a pull request there. Only what names this game — its disc id, addresses, save folders, quirks — belongs here. An unimplemented system call is almost always the framework's.
...
- **Trace the game; don't recall the hardware.** GE register numbers, PSP struct layouts and system call semantics from memory have been wrong before. Turn on the relevant `PURUN_TRACE_*` switch, read what the game does, then write the code.
- **Bound every run.** `timeout 90 out/bin/PurunNative`; never leave the game running or drive it with an open-ended input loop.
