...
#
# Needs an executable prepared by `PurunNative --install <image.iso>`, which
# writes EBOOT.ELF into the per-user data directory, or a game directory named
# by PURUN_GAME_DIR. Output: generated/, which is never committed.
#
...
if [[ -z "$elf" ]]; then
    for candidate in \
        "${PURUN_GAME_DIR:-}/EBOOT.ELF" \
        "${PURUN_DATA_DIR:-}/EBOOT.ELF" \
...
if [[ ! -f "$elf" ]]; then
    ...
    echo "Run '$build_dir/bin/PurunNative --install <image.iso>' first," >&2
    echo "or set PURUN_EBOOT to the decrypted EBOOT.ELF." >&2
    exit 1
fi
